...
class ModeApp:
    def __init__(self, root):
        ...
        # Bind event to reload image when switching to NOTES tab
        self.tab_control.bind("<<NotebookTabChanged>>", self.on_tab_changed)

    def on_tab_changed(self, event):
        # Check if the selected tab is NOTES
        selected_tab = self.tab_control.index(self.tab_control.select())
        if selected_tab == 0:  # NOTES is the first tab
...
class ModeAppTab212:
    def __init__(self, frame):
        ...
        if self.format_string:  # 如果 format_string 非空
            self.format_entry.insert(0, self.format_string)
        else:  # 預設值
    ...
    def load_settings(self):
        # Load settings from default.txt
        try:
            with open("default.txt", "r", encoding="utf-8") as f:
                lines = f.read().splitlines()
                region212 = list(lines[0]) if len(lines) > 0 else ["左", "中", "右"]
                items = list(lines[1]) if len(lines) > 1 else ["X", "椅", "沙", "壺", "鐘", "燈", "鏡"]

                # 如果第三行存在且非空，存到 self.format_string，否則設為空字串
                self.format_string = lines[2].strip() if len(lines) > 2 and lines[2].strip() else ""

                return region212, items
        except FileNotFoundError:
            # Default values if default.txt is not found
            self.format_string = ""  # 預設為空字串
            return ["左", "中", "右"], ["X", "椅", "沙", "壺", "鐘", "燈", "鏡"]
